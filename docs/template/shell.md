...
# https://cloud.tencent.com/developer/article/1835362
...
JAR_PATH='/opt/java'
# jar包名称
JAR_NAME=my-app-1.0-SNAPSHOT.jar
# PID 代表是PID文件
...
# java虚拟机启动参数
JAVA_OPTS="-Xms512m -Xmx512m -XX:MetaspaceSize=512m -XX:MaxMetaspaceSize=1024m -XX:ParallelGCThreads=4 -XX:+PrintGCDateStamps -XX:+PrintGCDetails -XX:+HeapDumpOnOutOfMemoryError -Dfile.encoding=utf-8 -Dserver.port=$SERVER_PORT"
...
start() {
  ...
  if [ $? -eq "1" ]; then
    ...
    nohup java $JAVA_OPTS -Xloggc:$LOG_FILE/gc/gclog.log -XX:HeapDumpPath=$LOG_FILE/gc/HeapDump.hprof -jar $JAR_PATH/$JAR_NAME &
    echo $! > $JAR_PID
